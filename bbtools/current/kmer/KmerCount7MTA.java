...
public class KmerCount7MTA extends KmerCountAbstract {
	...
	public static void main(String[] args){
		...
		try {
			counts=count(fname1, fname2, k, cbits, gap, true, counts);
		} catch (Exception e) {
...
//		System.out.println("d");
	...
	public static KCountArray count(String reads1, String reads2, int k, int cbits, int gap, boolean rcomp, KCountArray counts) throws Exception{
...
//			if(ff2!=null){ //TODO - interleaved flag
			cris=ConcurrentReadInputStream.getReadInputStream(maxReads, false, true, ff1, ff2);
			cris.start(); //4567
		}
...
//		assert(false) : THREADS;
		...
		for(int i=0; i<cta.length; i++){
			cta[i]=new CountThread(cris, k, rcomp, counts);
			cta[i].start();
		}
...
//		assert(false) : THREADS;
		CountThread[] cta=new CountThread[THREADS];
		for(int i=0; i<cta.length; i++){
			cta[i]=new CountThread(cris, k, rcomp, counts, trusted, thresh, detectStepsize, conservative);
			cta[i].start();
		}
...
//		System.out.println("\ncount=\n"+count);
	...
	private static class CountThread extends Thread{
		...
		CountThread(final ConcurrentReadInputStream cris_, final int k_, final boolean rcomp_, final KCountArray counts_){
			this(cris_, k_, rcomp_, counts_, null, 2, 1, true);
		}
...
//							}
							array=addRead_Advanced(r, counts, k, mask, array);
						}
