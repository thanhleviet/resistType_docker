...
//		index=new BBIndex(KEYLEN, minChrom, maxChrom, KFILTER, msa);
	...
	public final void postFilterRead(Read r){
		processIDFilter(r);
		processEditFilter(r);
	}
	...
	public final void processEditFilter(Read r){
...
//				}
		...
		while(!readlist.isEmpty()){
			...
			for(int i=0; i<readlist.size(); i++){
				...
				//if(r.mapLength!=r.length()){r.mapLength=r.length();}
				//if(r.mate!=null && r.mate.mapLength!=r.mateLength()){r.mate.mapLength=r.mateLength();}
				assert(r.mapLength==r.length()) : "Incorrect length: "+r.mapLength+"!="+r.length();
...
//			System.err.println("Adding list of length "+readlist.size());
	...
	public final int quickMap(final Read r, final byte[] basesM){
		...
		r.sites=list;
		removeOutOfBounds(r, DONT_OUTPUT_UNMAPPED_READS, (outStream!=null && outStream.ff1.samOrBam()), EXPECTED_LEN_LIMIT);
