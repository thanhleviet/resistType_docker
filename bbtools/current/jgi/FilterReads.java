...
public class FilterReads {
	...
	public static void main(String[] args){
		...
		if(TEST_KMERS){
			...
			KmerCountAbstract.maxReads=maxreads;
			int kbits=2*K;
			...
			MASK=~((-1L)<<(kbits));
			ht.stop();
			System.out.println("Finished hashing "+KmerCountAbstract.keysCounted+" kmers.");
		...
		final ConcurrentReadInputStream cris;
		{
			...
			cris=ConcurrentReadInputStream.getReadInputStream(-1, false, true, ff1, ff2);
			cris.start(); //4567
		}
...
//		assert(false) : cris.getClass();
	...
	public static long process(ConcurrentReadInputStream cris, String outfile, final long readsOut, final KCountArray kca2, 
...
//					System.out.print(", "+readsOut);
					...
					if(r.bases!=null && r.length()>=MIN_LEN_2 && (r.mate==null || (r.mate.bases!=null && r.length()>=MIN_LEN_2))){
...
//		}
	...
	public static void trim(Read r) {
		assert(r.length()>=MIN_LEN) : r.length();
		byte[] bases=r.bases;
...
//			r.clearSite();
		...
		r.mapLength=r.length();
		assert(r.length()>=MIN_LEN || r.discarded());
	}
	...
	public static boolean passesFilter2(Read r, KCountArray kca2){
		...
		if(r.bases==null || r.length()<MIN_LEN){removedShort++; return false;}
		if(r.avgQuality()<MIN_AVG_QUALITY){removedAvgQuality++; return false;}
		final int minq=r.minQualityLastNBases(r.length());
