...
//				System.err.println(r.id); //123
	...
	private void maskSam_MT(FileFormat ff){
		...
		{
			cris=ConcurrentReadInputStream.getReadInputStream(maxReads, colorspace, false, ff, null, null, null);
...
//		outstream.println("Spawning "+numThreads+" threads.");
...

					{
						...
						if(r.mapped()){
							...
							if(ref==null){
								...
								final int start=Tools.max(0, sl.start(false)-samPad);
								final int stop=Tools.min(sl.stop(start, false)+1+samPad, ref.length());
								if(stop>start){
									bs.set(start, stop);
								}
							}
	...
	private LinkedHashMap<String, Read> hashRef(){
		...
		final ConcurrentReadInputStream cris;
		{
			cris=ConcurrentReadInputStream.getReadInputStream(maxReads, colorspace, false, ffinRef, null, qfinRef, null);
			if(verbose){System.err.println("Started cris");}
			cris.start(); //4567
		}
