...
public class FindPrimers {
	...
	void process(Timer t){
		...
		{
			//Distributed version
//			ConcurrentReadInputStream cris0=ConcurrentReadInputStream.getReadInputStream(maxReads, false, false, ffin1, null);
//			cris=new ConcurrentReadInputStreamD(cris0, true);
			cris=ConcurrentReadInputStream.getReadInputStream(maxReads, false, false, ffin1, null);
		...
		MSA msa=MSA.makeMSA(maxqlen+3, 2000, false, "MultiStateAligner11ts");
			...
			while(reads!=null && reads.size()>0){
				...
				for(int idx=0; idx<reads.size(); idx++){
					...
					if(r.length()+2>msa.maxColumns){
						msa=MSA.makeMSA(maxqlen+3, r.length()+2+r.length()/2, false, "MultiStateAligner11ts");
					}
