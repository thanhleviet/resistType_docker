...
//			System.err.println("Processing "+args[i]);
			...
			if(Parser.isJavaFlag(arg)){
				...
				setbs=true;
			}else if(a.equals("array") || a.equals("arrays") || a.equals("usearrays") || a.equals("median") || a.equals("calcmedian")){
				bs=!Tools.parseBoolean(b);
				setbs=true;
...
//		assert(out!=null || outset) : "Output file was not set.";
	...
	public void process(){
		...
		ByteFile tf=ByteFile.makeByteFile(in, ReadWrite.USE_UNPIGZ, false);
...
//				}
	...
	public boolean addCoverage(final String scafName, final byte[] seq, final int start0, final int stop0, final int readlen, final int strand){
...
//			assert(sl.seq!=null) : sl.toString();
			final int length=sl.seq==null ? SamLine.calcCigarBases(sl.cigar, false, true) : sl.seq.length;
			final int start=sl.start(false);
			final int stop=sl.stop(start, false);
//			assert(false && length==stop-start+1) : length+", "+start+", "+stop+", "+(stop-start+1);
			return addCoverage(new String(sl.rname()), sl.seq, start, stop, length, sl.strand());
		}
		return false;
	}
