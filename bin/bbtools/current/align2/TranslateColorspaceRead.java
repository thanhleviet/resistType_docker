...
//		}
		...
		if(verbose){		
			...
			crbmq[0]=Arrays.copyOf(r.bases, r.length());
...
//		}else{assert(!containsNocalls(crbmq[3])) : "\n"+toString(crbmq)+"\n";}
		...
		if(indels>0 || (r2.stop-r2.start+1!=r2.length())){
			if(verbose){
...
//			}
			...
			final int maxI=msaBS.maxImperfectScore(r2.length());
			if(r2.strand()==Gene.PLUS){
				byte[][] matchR=new byte[1][];
				if(r2.match!=null && r2.match.length==r2.length()){
					matchR[0]=r2.match;
				}else{
//					System.err.println(new String(r2.match));
					matchR[0]=r2.match=new byte[r2.length()];
				}
...
//				assert(r2.stop==r2.start+r2.length()-1);
				r2.stop=r2.start+r2.length()-1;
			}else{
				byte[] bases=AminoAcid.reverseComplementBases(r2.bases);
...
//				assert(read.mapScore>0) : read.toText(false);
				v=new Varlet(read.chrom, read.strand(), rstart, rstop, mstart, mstop, varType, rs.toString(), cs.toString(),
						 ...
						 read.length(), read.mapLength, read.start, read.stop, read.copies, headDist, tailDist, endDist, 
						 read.pairnum());
...
//						}
						...
						Varlet v2=new Varlet(read.chrom, read.strand(), prev.beginLoc, v.endLoc, prev.matchStart, v.matchStop, varType,
								...
								(read.paired() ? 1 : 0), read.numericID, read.length(), read.mapLength,
