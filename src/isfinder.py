#!/usr/bin/env python
'''
Insertion Sequencing finding from the Insertion sequences downloaded from 
...
If the assembly is available, run from the contigs, otherwise look for fastq files an run ISFinder only on filtered reads
...
class IsFinder(object):
    def __init__(self, args):
        ...
        self.isfinderDir= _baseDir + "/resources/isdb/"
        self.alleleDB= self.isfinderDir + "ISFinderDatabase.fasta"
        ...
        self.blastFile = self.prefix + ".isfinder.blast.txt"
        self.outFile = self.prefix + ".isfinder.results.txt"
    ...
    def runMapAndSpades(self):
        ...
        self.miniFq1 = "{0}.isfinder_reads1.fq.gz".format(self.prefix)
        self.miniFq2 = "{0}.isfinder_reads2.fq.gz".format(self.prefix)
        cmdLine = "bowtie2 --fast -x {0}  -1 {1} -2 {2}|python {3} {4} {5} ".format(self.alleleDB.split(".fa")[0] + "_padded.fa", self.fq1, self.fq2 , filterScript, self.miniFq1, self.miniFq2)
        print cmdLine
        os.system(cmdLine)
...
if __name__ == "__main__":
    ...
    IsFinderModule= IsFinder([opts.sampleName, opts.contigFile, opts.outDir, opts.fq1, opts.fq2])
